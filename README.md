...
**なぜ重要か？**
迷彩パターンの有効性は「誰が見ても同じ」ではありません。
色覚多様性（色覚異常、色弱）を持つ人の視点では、通常視覚とはまったく異なる見え方になります。

赤緑色覚異常（1型・2型）や青系の識別が弱い3型など、異なる色覚特性の視点では、設計者が想定していないコントラストが強調されてしまうことがあります。

実際、赤緑色弱の観察者には、通常視力者には見えない「明度差」が強調され、一部の迷彩が逆に目立つ事例も知られています。
...
### ハードウェア群パターン（Hardware Panel）　【実装予定】
...
- **考察**：漆黒パターンで全体を覆えば暗所では目立たなくなるが、基板の端子やLEDが強い「人工的シグナル」として浮き出る。  
...
- **教育的ポイント**：背景に同化する色や模様があっても、通信挙動やLEDランプの点滅が決定的な発見手がかりとなる。
...
迷彩（Camouflage）は単なる模様・パターンではなく、**色相・明度・彩度（色の三要素）** を精密に調整することで背景環境に溶け込む効果を高められます。
つまり、迷彩デザインのためには色彩理論を学ぶことが有益です。

### 色の三要素と迷彩効果

#### 色相（Hue）

背景と同じ「色相帯」を使うほど、迷彩は視覚に自然に溶け込みます。
森林では緑・茶、砂漠ではタン（黄土）やグレージュ、雪原では白系統がベースになります。

背景との色相差が大きいと、どれだけ複雑なパターンであっても輪郭が浮き上がってしまいます。

#### 明度（Brightness）

明度は迷彩効果の中でもとくに重要です。
人間の視覚は薄暗くなると色相認識が弱まり、明度コントラストだけが残るため、夜間や暗部では「色よりも明るさの差」が勝負になります。

迷彩パターンで明度のバランスを外すと、暗所で輪郭が浮いてしまうため、背景の明度に合わせた調整が不可欠です。

#### 彩度（Saturation）

彩度は「鮮やかさ」です。
砂埃の多い地域ではくすんだ低彩度色、雨上がりの森では鮮やかな高彩度色が背景を構成します。

彩度の調整を誤ると、背景の雰囲気から浮く「色のノイズ」となり、迷彩効果を損ないます。

### Color Hack Boxとの連携

Color Hack Boxは色彩理論を体感的に学ぶインタラクティブツールです。

- [Color Hack Box](https://ipusiron.github.io/color-hack-box/) - 色彩理論を体感的に学ぶインタラクティブツール（Day101）

CamoForgeが迷彩パターンの「形」、Color Hack Boxが色彩理論に基づく「色」を担当します。
形と色をうまく調整することで、以下のようなより実戦的な迷彩設計が可能になります。
...
迷彩の本質は背景に見えることです。
そのためには、パターンと色彩の両輪が必要であり、CamoForgeとColor Hack Boxはまさにそのためのセットツールとして機能します。
